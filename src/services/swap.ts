...
export function buildSwapUSDT0ToXAUT0Calls(amountIn: bigint, amountOutMinimum: bigint, recipient: Address): Call[] {
    const calls: Call[] = [];

    // 1. Approve USDT0 to router
    calls.push(buildApproveCall(CONTRACTS.USDT0, CONTRACTS.UNISWAP_SWAP_ROUTER, amountIn));

    // 2. Single-hop swap: USDT0 -> XAUT0
    calls.push(
...
export function buildSwapXAUT0ToUSDT0Calls(amountIn: bigint, amountOutMinimum: bigint, recipient: Address): Call[] {
    const calls: Call[] = [];

    // 1. Approve XAUT0 to router
    calls.push(buildApproveCall(CONTRACTS.XAUT0, CONTRACTS.UNISWAP_SWAP_ROUTER, amountIn));

    // 2. Single-hop swap: XAUT0 -> USDT0
    calls.push(
        buildExactInputSingleCall({
...
export function calculateMinAmountOut(amountOut: bigint, slippageTolerance: number = DEFAULT_SLIPPAGE): bigint {
 ...
 */
export async function estimateSwapOutput(tokenIn: Address, tokenOut: Address, amountIn: bigint): Promise<bigint> {

...
export function SwapCard() {
  ...
  const [direction, setDirection] = useState<"USDT0_TO_XAUT0" | "XAUT0_TO_USDT0">(
    "USDT0_TO_XAUT0"
  );
  ...
  const handleFlipDirection = () => {
    setDirection((prev) =>
      prev === "USDT0_TO_XAUT0" ? "XAUT0_TO_USDT0" : "USDT0_TO_XAUT0"
    );
    setAmountIn("");
    setEstimatedOut(BigInt(0));
    resetState();
  ...
  const hasInsufficientBalance =
    parsedAmount && balanceIn && parsedAmount > balanceIn;
  const canSwap =
    parsedAmount &&
  ...
  return (
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 flex flex-col gap-4">
          ...
          <div className="space-y-4 min-w-0">
              ...
              <div className="bg-gray-900 rounded-lg p-4 border border-gray-700">
                  ...
                  <div className="flex items-center gap-4">
                      <span className="flex-1 text-2xl text-white">
                          {estimatedOut > BigInt(0) ? formatTokenAmount(estimatedOut, tokenOut.decimals, 6) : "0.00"}
                      </span>
              ...
              {/* Error message */}
              {hasInsufficientBalance && <p className="text-sm text-red-400">Insufficient {tokenIn.symbol} balance</p>}

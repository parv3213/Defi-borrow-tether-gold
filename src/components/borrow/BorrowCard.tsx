...
export function BorrowCard() {
  ...
  const exceedsMaxBorrow = borrowAmount && borrowAmount > maxBorrowAmount;
  const isLTVWarning = projectedLTV > WARNING_LTV;
  const isLTVDanger = projectedLTV > (market?.lltv || 0.77);
          ...
          <div className="space-y-4 flex-1 flex flex-col">
              ...
              <div className="space-y-2">
                  ...
                  {safeBorrowAmount > BigInt(0) && (
                      <button onClick={handleSafeBorrow} className="text-sm text-indigo-400 hover:text-indigo-300">
                          Suggested safe amount: {formatTokenAmount(safeBorrowAmount, TOKENS.USDT0.decimals)} USDT0
                      </button>
                  )}
              </div>
              ...
              {/* Error messages */}
              {hasInsufficientCollateral && <p className="text-sm text-red-400">Insufficient XAUT0 balance</p>}
              {exceedsMaxBorrow && <p className="text-sm text-red-400">Borrow amount exceeds maximum LTV</p>}
              {isLTVWarning && !isLTVDanger && (
                  <p className="text-sm text-yellow-400">⚠️ High LTV - Risk of liquidation if XAUT price drops</p>
              )}
              {isLTVDanger && <p className="text-sm text-red-400">❌ LTV exceeds liquidation threshold</p>}

              {/* Submit button */}
              <div className="mt-auto">
                  <TransactionButton onClick={handleBorrow} state={txState} disabled={!canBorrow}>
                      {borrowAmount && borrowAmount > BigInt(0) ? "Supply Collateral & Borrow" : "Supply Collateral"}
                  </TransactionButton>
              </div>
          </div>
      </div>
